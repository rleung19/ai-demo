#!/bin/bash
# Start both Next.js frontend and standalone API server
# Usage: ./scripts/start-all.sh

set -e

# Colors for output
GREEN='\033[0;32m'
BLUE='\033[0;34m'
YELLOW='\033[1;33m'
NC='\033[0m' # No Color

echo -e "${BLUE}============================================================${NC}"
echo -e "${BLUE}Starting AI Demo Application${NC}"
echo -e "${BLUE}============================================================${NC}"

# Load environment variables
if [ -f .env ]; then
  export $(cat .env | grep -v '^#' | xargs)
  echo -e "${GREEN}✓ Loaded .env file${NC}"
else
  echo -e "${YELLOW}⚠️  .env file not found${NC}"
fi

# Ensure TNS_ADMIN is set
if [ -z "$TNS_ADMIN" ] && [ -n "$ADB_WALLET_PATH" ]; then
  export TNS_ADMIN="$ADB_WALLET_PATH"
  echo -e "${GREEN}✓ Set TNS_ADMIN to: $TNS_ADMIN${NC}"
fi

# Set API port if not set
if [ -z "$API_PORT" ]; then
  export API_PORT=3001
fi

echo ""
echo -e "${BLUE}Configuration:${NC}"
echo "  Next.js Frontend: http://localhost:3000"
echo "  API Server:       http://localhost:$API_PORT"
echo "  TNS_ADMIN:        ${TNS_ADMIN:-not set}"
echo ""

# Function to cleanup on exit
cleanup() {
  echo ""
  echo -e "${YELLOW}Shutting down servers...${NC}"
  kill $NEXTJS_PID $API_PID 2>/dev/null || true
  exit
}

trap cleanup SIGINT SIGTERM

# Start API server in background
echo -e "${BLUE}Starting API server on port $API_PORT...${NC}"
API_PORT=$API_PORT npm run server:dev > /tmp/api-server.log 2>&1 &
API_PID=$!
echo -e "${GREEN}✓ API server started (PID: $API_PID)${NC}"
echo "  Logs: /tmp/api-server.log"

# Wait a bit for API server to start
sleep 3

# Start Next.js in background
echo -e "${BLUE}Starting Next.js frontend on port 3000...${NC}"
npm run dev > /tmp/nextjs.log 2>&1 &
NEXTJS_PID=$!
echo -e "${GREEN}✓ Next.js started (PID: $NEXTJS_PID)${NC}"
echo "  Logs: /tmp/nextjs.log"

echo ""
echo -e "${GREEN}============================================================${NC}"
echo -e "${GREEN}Both servers are running!${NC}"
echo -e "${GREEN}============================================================${NC}"
echo ""
echo "Frontend:  http://localhost:3000"
echo "API:       http://localhost:$API_PORT"
echo ""
echo "Press Ctrl+C to stop both servers"
echo ""

# Wait for both processes
wait
